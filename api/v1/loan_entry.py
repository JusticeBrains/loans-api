...
async def get_loan_entries(session: AsyncSession = Depends(get_session)):
    return await LoanEntriesService.get_loan_entries(session=session)


@router.get("/{id}", response_model=LoanEntriesRead, status_code=status.HTTP_200_OK)
async def get_loan_entry(id: UUID, session: AsyncSession = Depends(get_session)):
...
async def create_loan_entry(
    data: LoanEntriesCreate, session: AsyncSession = Depends(get_session)
):
...
async def update_loan_entry(
    id: UUID, data: LoanEntriesUpdate, session: AsyncSession = Depends(get_session)
):
...
async def delete_loan_entries(id: UUID, session: AsyncSession = Depends(get_session)):
